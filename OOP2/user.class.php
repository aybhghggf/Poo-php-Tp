...
class User
{
    ...
    {
        global $pdo;
        $stmt = $pdo->prepare("INSERT INTO users(Nom,Prenom,Email,Telephone,Pass) VALUES (?,?,?,?,?)");
...
public static function Login($email, $password)
{
    ...
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    if($user['Email'] === $email && $user['Pass'] === $password || password_verify($password, $user['Pass'])) {
        session_start();
        $_SESSION['user'] = new User( $user['Nom'],$user['Prenom'], $user['Telephone'], $user['Email'], $user['Pass']);;
